...
To give an example, consider the TM that prints three "1"s. Once the TM reaches the Stop state, its last state snapshot is "$\$1|100|11\$$". One possible tape contraction sequence is:
...
In the general case, we can give a sketch of what a complete balanced sequence corresponding to a valid TM computation looks like:
